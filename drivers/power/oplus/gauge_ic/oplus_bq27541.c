...
/* Jiaoyang@CM.BSP.CHG 2020/10/30 add for battery gauge authentication */
...
static int bq28z610_set_sleep_mode_withoutunseal(bool val) {
	int rc = -1;
	...
	bq27541_i2c_txsubcmd(BQ28Z610_REG_CNTL1, BQ28Z610_CNTL1_DA_CONFIG);
	usleep_range(10000, 10000);
	bq27541_read_i2c_block(BQ28Z610_REG_CNTL1, BQ28Z610_REG_I2C_SIZE, read_data);
	...
	if (val) {
		...
		rc = bq27541_write_i2c_block(BQ28Z610_REG_CNTL1, BQ28Z610_REG_I2C_SIZE, &enable_data[0]);
		msleep(10);
		bq27541_i2c_txsubcmd(BQ28Z610_REG_CNTL2, 0x05ab);
		msleep(10);
	} else {
		pr_err("%s disable sleep_mode, val=%d\n", __func__, val);
		rc = bq27541_write_i2c_block(BQ28Z610_REG_CNTL1, BQ28Z610_REG_I2C_SIZE, &disable_data[0]);
		msleep(10);
		bq27541_i2c_txsubcmd(BQ28Z610_REG_CNTL2, 0x05bb);
		msleep(10);
	}
	return rc;
}

static int bq28z610_set_sleep_mode(bool val)
{
	int rc = -1;
	...
	if(gauge_ic->batt_bq28z610) {
		...
		rc = bq28z610_set_sleep_mode_withoutunseal(val);
		if (bq8z610_sealed() == 0) {
			usleep_range(1000, 1000);
			bq8z610_seal();
		}
		...
		(struct chip_bq27541 *chip, bool is_powerup)
{
	...
	if(gauge_ic->batt_bq28z610 && is_powerup == true) {
		pr_err("%s batt_bq28z610 powerup\n", __func__);
		//power up disable sleep mode
		bq28z610_set_sleep_mode(false);
		return GUAGE_OK;
	}
...
write_parameter:
	...
	rc = bq28z610_set_sleep_mode_withoutunseal(true);
	if (rc && tried_again == false) {
		tried_again = true;
		goto write_parameter;
	}
