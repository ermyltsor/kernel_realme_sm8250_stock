...
void oplus_vooc_fw_type_dt(struct oplus_vooc_chip *chip)
{
	...
	rc = of_property_read_u32(node, "qcom,vooc_mcu_reset_delay",
		&chip->vooc_mcu_reset_delay);
	if (rc) {
		chip->vooc_mcu_reset_delay = 2500;
	} else {
		chg_debug("qcom,vooc_mcu_reset_delay is %dus\n",
			chip->vooc_mcu_reset_delay);
	}
}
...
void opchg_set_reset_active_force(struct oplus_vooc_chip *chip)
{
	...
	usleep_range(chip->vooc_mcu_reset_delay, chip->vooc_mcu_reset_delay); //2500
