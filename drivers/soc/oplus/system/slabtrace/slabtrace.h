...
static void *oplus_fixup_red_left(struct kmem_cache *s, void *p);
static inline void *oplus_get_freepointer(struct kmem_cache *s, void *object);
static inline unsigned int oplus_slab_index(void *p, struct kmem_cache *s, void *addr);
static int oplus_alloc_loc_track(struct loc_track *t, unsigned long max, gfp_t flags);
...
#if IS_MODULE(CONFIG_OPLUS_FEATURE_SLABTRACE_DEBUG)
LIST_HEAD(slab_caches);
DEFINE_MUTEX(slab_mutex);
#endif

#define for_each_object(__p, __s, __addr, __objects) \
	for (__p = oplus_fixup_red_left(__s, __addr); \
